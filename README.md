...
```bash
(cd /tmp && ([[ -d sexy-bash-prompt ]] || git clone --depth 1 --config core.autocrlf=false https://github.com/brmvnml/bram-bash-prompt) && cd sexy-bash-prompt && make install) && source ~/.bashrc
